...
(*
 * Formal TLA+ specification of an IBFT-style consensus protocol
 * featuring F-skip leader rotation to enhance censorship resistance.
 *
 * Key properties:
 *   - Safety (Agreement, Validity) under up to F Byzantine failures
 *   - Liveness under partial synchrony (assumed via fairness)
 *   - Chain quality: in any (F+1) consecutive blocks, at least one is proposed by a correct process
 *
 *)
...
CONSTANT
    ...
    Byzantine,     \* Set of Byzantine (malicious) processes (may be empty)
    N,             \* Total number of processes: N = |Corr ∪ Byzantine|
    MaxView,       \* Maximum view number per height (for bounded model checking)
    MaxHeight,     \* Maximum block height (used for chain quality property)
...
ASSUME
    ...
    /\ MaxHeight >= 1
    /\ MaxHeight >= Cardinality(Byzantine) + 1  \* Required for chain quality
    /\ (Corr \cup Byzantine) \subseteq Int       \* Processes are integers (for ordering)
...
VARIABLES
    messages,         \* Set of PRE-PREPARE, PREPARE, COMMIT messages for current height
    processState,     \* Per-process state (view, input, last prepared)
    decision,         \* Decided values per height and per correct process
    RCmessages,       \* Set of ROUND-CHANGE messages for current height
...
QUORUM   == (2 * F) + 1  \* Minimal quorum size for safety
...
(* Minimum element of a set (used for deterministic leader selection) *)
Min(S) == CHOOSE x \in S : \A y \in S : x <= y

(* k-th smallest element in a finite set of integers *)
KthMin(S, k) == CHOOSE x \in S : Cardinality({ y \in S : y < x }) = k - 1

(***************************************************************************)
(*                     F-SKIP LEADER SELECTION LOGIC                         *)
(* Ensures that the last F leaders are excluded from the candidate set    *)
(***************************************************************************)

LastFLeaders(h) ==
    IF h <= F THEN {}
    ELSE { blockLeader[i] : i \in (h - F)..(h - 1) \cap committedHeights }

AllowedLeaders(h) == AllProcs \ LastFLeaders(h)

Leader(h, v) ==
    LET candidates == AllowedLeaders(h) IN
    IF candidates = {} THEN CHOOSE p \in AllProcs : TRUE  \* Fallback (should not occur under assumptions)
    ELSE
        LET n   == Cardinality(candidates)
            idx == ((v - 1) % n) + 1  \* Round-robin over allowed leaders
        IN
        KthMin(candidates, idx)
...
Init ==
    /\ \E iv \in [Corr -> Values] :  \* Each correct process has an input value
        LET leader1 == Leader(1, 1) IN
        LET initialCmd ==
            IF leader1 \in Corr THEN iv[leader1]
            ELSE CHOOSE c \in Values : TRUE  \* Byzantine leader picks arbitrary value
        IN
        /\ leader1 \in AllProcs
        ...
        /\ processState = [p \in Corr |->
                             [view_num |-> 1,
                              inputValue |-> iv[p],
                              pr |-> NilView,      \* prepared round
                              pv |-> NilValue]]    \* prepared value
...
(* Extract the highest prepared (pr, pv) pair from a set of ROUND-CHANGE messages *)
HighestPrepared(Q) ==
...
(* A quorum of ROUND-CHANGE messages justifies a view change *)
...
(* A PRE-PREPARE message is valid iff: *)
(*  - It comes from the expected leader *)
(*  - For view > 1, it is justified by a quorum of ROUND-CHANGE messages *)
JustifyPrePrepare(m) ==
    /\ m.type = "PRE-PREPARE"
    /\ m.cmd \in Values
...
Timeout(p) ==  \* Triggered when a correct process makes no progress in current view
    /\ p \in Corr
    /\ currentHeight <= MaxHeight
    /\ messageCount < MaxMessages
...
(* Each action models a distinct adversarial behavior                      *)
(***************************************************************************)

ByzantineLeaderPrepares(p) ==  \* Sends a valid-looking PRE-PREPARE (any value)
    /\ p \in Byzantine
    /\ currentHeight <= MaxHeight
    /\ messageCount < MaxMessages
    /\ \E v \in Views :
...
ByzantineLeaderIgnoresJustification(p) ==  \* Sends PRE-PREPARE contradicting ROUND-CHANGE justification
    /\ p \in Byzantine
    /\ currentHeight <= MaxHeight
    /\ messageCount < MaxMessages
    /\ \E v \in Views :
...
ByzantineInvalidRC(p) ==  \* Sends ROUND-CHANGE with neutral (Nil) payload
    /\ p \in Byzantine
    /\ currentHeight <= MaxHeight
    /\ messageCount < MaxMessages
    /\ \E v \in Views :
        ...
        /\ RCmessages' = RCmessages \cup {
              [type |-> "ROUND-CHANGE", view_num |-> v, from |-> p,
               pr |-> NilView, pv |-> NilValue]
           }
        /\ messageCount' = messageCount + 1
        /\ UNCHANGED <<messages, processState, decision, committedHeights, blockLeader, currentHeight>>

ByzantineFakeLeader(p) ==  \* Impersonates a leader (sends PRE-PREPARE as non-leader)
    /\ p \in Byzantine
    /\ currentHeight <= MaxHeight
    /\ messageCount < MaxMessages
    /\ \E v \in Views :
        ...
        /\ ~\E m \in messages : m.type = "PRE-PREPARE" /\ m.from = p /\ m.view_num = v
        /\ messages' = messages \cup {[type |-> "PRE-PREPARE", view_num |-> v,
                                       cmd |-> "A", from |-> p]}  \* Fixed malicious value
        /\ messageCount' = messageCount + 1
        /\ UNCHANGED <<RCmessages, processState, decision, committedHeights, blockLeader, currentHeight>>
